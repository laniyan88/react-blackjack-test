describe('Blackjack happy path', () => {
  ...
  it('card-back selection persists', () => {
    ...
    cy.reload();
    cy.findByRole('button', { name: /card back purple/i })
  ...
  it('bet step buttons clamp and respect min/max', () => {
    ...
    // 10 → -5 → -5 → 1
    cy.findByRole('button', { name: /decrease bet by 5/i }).click().click();
  ...
  it('stats increment and reset', () => {
    ...
    const statVal = (label: string) =>
      cy.findByRole('region', { name: /game statistics/i })
      .contains('span', label)
      .parent()
      .find('span')
      .eq(1); 
  ...
  it('card-back is single-select', () => {
    cy.visit('http://localhost:3000/');
    cy.findByRole('button', { name: /card back red/i }).click()
      .should('have.attr', 'aria-pressed', 'true');
    cy.findByRole('button', { name: /card back blue/i }).click()
      .should('have.attr', 'aria-pressed', 'true');
